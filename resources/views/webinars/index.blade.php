                        ...
                        <tbody>
                            @forelse($webinars as $w)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $webinarDateTime = \Carbon\Carbon::parse($w->date . ' ' . $w->time);
                                        @endphp

                                        @if($webinarDateTime->isPast())
                                            <span class="badge bg-secondary">✅ Completed</span>
                                        @elseif($w->link)
                                            <a href="{{ $w->link }}" class="btn btn-outline-primary btn-sm" target="_blank">🔗
                                                Join</a>
                                        @else
                                    ...
                                    <td>
                                        <a href="{{ route('webinars.edit', $w->id) }}" class="btn btn-primary btn-sm">✏
                                            Edit</a>
                                        <form action="{{ route('webinars.destroy', $w->id) }}" method="POST"
                                            style="display:inline;">
                                            @csrf @method('DELETE')
                                            <button class="btn btn-danger btn-sm"
                                                onclick="return confirm('Delete this webinar?')">🗑 Delete</button>
                                        </form>
                                    </td>
                                </tr>
